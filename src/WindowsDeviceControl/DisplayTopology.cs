...
public static partial class DisplayTopology
{
    ...
    public static DisplayTopologySnapshot CaptureActive()
    {
        ...
        {
            var identity = ReadTarget(path);
            result.Add(new ActiveDisplayPath(identity, ReadSourceName(path), path.TargetInfo.OutputTechnology,
        ...
        DisplayTargetIdentity identity, TimeSpan timeout, CancellationToken cancellationToken = default)
    {
        ...
        var outcome = await PollAsync(timeout, () =>
        {
    ...
    internal static void ValidateBufferCounts(uint paths, uint modes)
    {
        if (paths > 4096 || modes > 8192)
        {
            throw new InvalidOperationException(
                $"Display topology exceeds supported bounds ({paths} paths, {modes} modes).");
        }
    }

    /// <summary>
    ///     Finds the active path currently driving one monitor. The route changes on hotplug, so
    ///     it is resolved per call rather than stored.
    /// </summary>
    ...
    internal static bool TryFindActive(DisplayTargetIdentity target, out PathInfo path)
    {
        ...
        {
            var found = false;
            foreach (var candidate in Query(OnlyActivePaths, "Active display topology query failed.").Paths)
            {
                // Every active target is read, so an unreadable display still fails the lookup.
                var identity = ReadTarget(candidate);
                if (!found && target.Matches(identity))
                {
                    path = candidate;
                    found = true;
                }
