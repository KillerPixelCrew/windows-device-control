...
public static class DisplayEdid
{
    ...
    [
        new(720, 400, 70), new(720, 400, 88), new(640, 480, 60),
    ...
    internal static IReadOnlyList<DisplayMode> Parse(ReadOnlySpan<byte> edid)
    {
        ...
        for (var bit = 0; bit < Established.Length; bit++)
        {
            // The 1024x768@87 entry is interlaced, which DisplayMode cannot represent.
            if (bit != 11 && (edid[35 + bit / 8] & (128 >> (bit % 8))) != 0)
            {
                modes.Add(Established[bit]);
            }
        }
        ...
        var blocks = Math.Min(edid[126], edid.Length / 128 - 1);
        for (var index = 1; index <= blocks; index++)
        {
            var block = edid.Slice(index * 128, 128);
            if (!Checksum(block))
            {
                continue;
            }
            ...
            else if (block[0] == 0x70)
            {
                AddDisplayId(block, modes);
            }
        }
    ...
    {
        for (var offset = 0; offset + 2 <= timings.Length; offset += 2)
        {
            if (timings[offset] < 2)
            {
                continue;
            ...
            var width = (timings[offset] + 31) * 8;
            var height = (timings[offset + 1] >> 6) switch
            {
                0 => revision < 3 ? width : width * 10 / 16, 1 => width * 3 / 4, 2 => width * 4 / 5,
                _ => width * 9 / 16
            };
            modes.Add(new DisplayMode(width, height, (timings[offset + 1] & 63) + 60));
        }
    }

    private static void AddDetailed(ReadOnlySpan<byte> timing, HashSet<DisplayMode> modes)
    {
        var clock = timing[0] | (timing[1] << 8);
        if (clock == 0 || (timing[17] & 128) != 0)
        {
            return;
        }

        var width = timing[2] | ((timing[4] & 240) << 4);
        var height = timing[5] | ((timing[7] & 240) << 4);
        var blankX = timing[3] | ((timing[4] & 15) << 8);
        var blankY = timing[6] | ((timing[7] & 15) << 8);
    ...
    {
        if (width <= 0 || height <= 0 || blankX <= 0 || blankY <= 0)
        {
            return;
        }

        var hz = (int)Math.Round((double)clock / ((long)(width + blankX) * (height + blankY)));
        if (hz >= 2 && hz <= 1000)
        {
            modes.Add(new DisplayMode(width, height, hz));
        }
    }
    ...
    private static void AddCta(ReadOnlySpan<byte> block, HashSet<DisplayMode> modes)
    {
        ...
        for (var offset = 4; offset < end;)
        {
            ...
            if (tag == 2)
            {
                ...
                {
                    var vic = block[index] is >= 129 and <= 192 ? block[index] & 127 : block[index];
                    if (CtaMode(vic) is { } mode)
                    {
                        modes.Add(mode);
                    }
                }
        ...
        for (var offset = end; offset + 18 <= 127; offset += 18)
        {
            AddDetailed(block.Slice(offset, 18), modes);
        }
    }

    private static DisplayMode? CtaMode(int vic)
    {
        return vic switch
        {
            1 => new DisplayMode(640, 480, 60),
            2 or 3 => new DisplayMode(720, 480, 60),
            4 => new DisplayMode(1280, 720, 60),
            16 => new DisplayMode(1920, 1080, 60),
            17 or 18 => new DisplayMode(720, 576, 50),
            19 => new DisplayMode(1280, 720, 50),
            31 => new DisplayMode(1920, 1080, 50),
            32 => new DisplayMode(1920, 1080, 24),
            33 => new DisplayMode(1920, 1080, 25),
            34 => new DisplayMode(1920, 1080, 30),
            41 => new DisplayMode(1280, 720, 100),
            42 or 43 => new DisplayMode(720, 576, 100),
            47 => new DisplayMode(1280, 720, 120),
            48 or 49 => new DisplayMode(720, 480, 120),
            52 or 53 => new DisplayMode(720, 576, 200),
            56 or 57 => new DisplayMode(720, 480, 240),
            60 => new DisplayMode(1280, 720, 24),
            61 => new DisplayMode(1280, 720, 25),
            62 => new DisplayMode(1280, 720, 30),
            63 => new DisplayMode(1920, 1080, 120),
            64 => new DisplayMode(1920, 1080, 100),
            >= 65 and <= 71 => new DisplayMode(1280, 720, VideoRate(vic - 65)),
            >= 72 and <= 78 => new DisplayMode(1920, 1080, VideoRate(vic - 72)),
            >= 79 and <= 85 => new DisplayMode(1680, 720, VideoRate(vic - 79)),
            >= 86 and <= 92 => new DisplayMode(2560, 1080, VideoRate(vic - 86)),
            >= 93 and <= 97 => new DisplayMode(3840, 2160, VideoRate(vic - 93)),
            >= 98 and <= 102 => new DisplayMode(4096, 2160, VideoRate(vic - 98)),
            >= 103 and <= 107 => new DisplayMode(3840, 2160, VideoRate(vic - 103)),
            108 or 109 => new DisplayMode(1280, 720, 48),
            110 => new DisplayMode(1680, 720, 48),
            111 or 112 => new DisplayMode(1920, 1080, 48),
            113 => new DisplayMode(2560, 1080, 48),
            114 or 116 => new DisplayMode(3840, 2160, 48),
            115 => new DisplayMode(4096, 2160, 48),
            117 or 119 => new DisplayMode(3840, 2160, 100),
            118 or 120 => new DisplayMode(3840, 2160, 120),
            _ => null
        };
    }

    private static int VideoRate(int index)
    {
        return index switch { 0 => 24, 1 => 25, 2 => 30, 3 => 50, 4 => 60, 5 => 100, _ => 120 };
    }

    private static void AddDisplayId(ReadOnlySpan<byte> block, HashSet<DisplayMode> modes)
    {
        var end = 5 + block[2];
        var version = block[1] >> 4;
        if (version is not (1 or 2) || end > 126 || !Checksum(block.Slice(1, end)))
        {
            return;
        }

        for (var offset = 5; offset + 3 <= end;)
        {
            ...
            {
                for (var index = offset; index < offset + length; index += 20)
                {
                    var timing = block.Slice(index, 20);
                    if ((timing[3] & 16) != 0)
                    {
                        continue;
                    }
    ...
    private static int Word(ReadOnlySpan<byte> data, int offset)
    {
