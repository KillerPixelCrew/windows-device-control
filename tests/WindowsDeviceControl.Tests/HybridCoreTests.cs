...
public sealed class HybridCoreTests
{
    ...
    public void GroupsLogicalProcessorsAndDistinctCoresByEfficiencyClass()
    {
        ...
        var classes = WindowsPower.ParseCpuSets(buffer);

        Assert.Equal([new HybridCoreClass(0, 2, 2), new HybridCoreClass(1, 1, 2)], classes);
    }

    [Fact]
    public void SeparatesEqualCoreIndexesInDifferentProcessorGroups()
    {
        var buffer = Buffer((group: 0, core: 3, efficiency: 0), (group: 1, core: 3, efficiency: 0));
    ...
    {
        var buffer = Buffer((group: 0, core: 0, efficiency: 2));
        var mixed = new byte[EntryBytes + buffer.Length];
    ...
    [Fact]
    public void EmptyInformationReportsNoClasses()
    {
        Assert.Empty(WindowsPower.ParseCpuSets([]));
    }
    ...
    {
        var buffer = Buffer((group: 0, core: 0, efficiency: 0));
    ...
    {
        var buffer = new byte[processors.Length * EntryBytes];
        for (var index = 0; index < processors.Length; index++)
        {
            var entry = buffer.AsSpan(index * EntryBytes, EntryBytes);
