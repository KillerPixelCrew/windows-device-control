...
public sealed class DisplayEdidTests
{
    ...
    public void BaseTimingsAreDecodedAndInvalidDescriptorsAreRejected()
    {
        var edid = Descriptor();
        edid[38] = 209;
        edid[39] = 192; // 1920x1080 at 60 Hz standard timing.
    ...
    public void CtaOffersProgressiveVideoModesAndRejectsBrokenExtensionChecksums()
    {
        ...
        Assert.Equal(new[] { new DisplayMode(1920, 1080, 60), new DisplayMode(3840, 2160, 60) },
            DisplayEdid.Parse(edid));
        edid[134]++;
        Assert.Empty(DisplayEdid.Parse(edid));
    }
    ...
    {
        var edid = Descriptor(1);
        var block = edid.AsSpan(128, 128);
        block[0] = 0x70;
        block[1] = (byte)(tag == 3 ? 0x12 : 0x20);
        block[2] = 23;
        block[5] = (byte)tag;
        block[7] = 20;
        var timing = block.Slice(8, 20);
        timing[0] = (byte)clock;
        timing[1] = (byte)(clock >> 8);
        timing[2] = (byte)(clock >> 16);
